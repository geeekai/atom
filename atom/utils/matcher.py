        ...
        for i, rel1 in enumerate(rels1):
            if best_scores[i] >= threshold:
                ...
                if rels1[i].name == rels2[j].name:
                    print(f"[INFO] Wohoo! Relation --- [{rels1[i].name}] -- exists already in the global relationships")
                else:
                    print(f"[INFO] Wohoo! Relation was matched --- [{rels1[i].name}] --merged --> [{rels2[j].name}] ")
                # Rename rels1[i] to the best matching name in rels2[j]
                rels1[i].name = rels2[j].name
                # We do NOT change startEntity/endEntity!
        
            # - Matching relationshipps by timestamps -------------------------------------
            if rel1 in rels2:

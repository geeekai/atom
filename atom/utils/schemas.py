...
class Entity(BaseModel):
    label: str = Field(
        description=(
            "The type or category of the entity (e.g., 'Person', 'Event', 'Location', 'Methodology'). "
            "This field is primarily used for classification within the knowledge graph. If a concept "
            "is inherently relational (like a verb), consider using a Relationship instead of an Entity."
        )
    )
    name: str = Field(
        description=(
            "The specific, single concept this entity represents (e.g., 'Haira', 'Escape', 'Prison'). "
            "Avoid combining multiple ideas into one name (e.g., 'plans an escape'); verbs or complex "
            "phrases typically belong either in the Relationship or should be split into multiple entities."
        )
    )
...
class EntitiesExtractor(BaseModel):
    entities: List[Entity] = Field(
        description=(
            "All the distinct entities identified from the context, each encoding exactly one concept. "
            "For instance, a phrase like 'plans and executes an escape from prison' might yield separate "
            "entities such as Person('Haira'), Event('Escape'), and Location('Prison'). Verbs (like 'plans', "
            "'executes') usually appear in Relationship objects rather than as entities."
        )
    )
...
class Relationship(BaseModel):
    startNode: Entity = Field(
        description=(
            "The 'subject' or source entity of the relationship, which must be one of the extracted entities. "
            "For example, if the relationship is 'plans', the startNode might be the Person who does the planning."
        )
    )
    endNode: Entity = Field(
        description=(
            "The 'object' or target entity of the relationship, also one of the extracted entities. "
            "Using the same 'plans' example, the endNode might be the Event or activity that was planned."
        )
    )
    name: str = Field(
        description=(
            "A single, semantically distinct predicate capturing how the startNode and endNode relate. "
            "Typical values might be 'plans', 'executes', 'results_in', or 'from'. Avoid combining multiple "
            "relations in one field (e.g., 'plans_and_executes')."
        )
    )
